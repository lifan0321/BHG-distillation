...
from models.resnet_cls_all_9_no_grad import AllInOne_Dist
...
def main():
    ...
    pretrain_path_allone = os.path.join(opt.pretrain_path_allone, str(opt.fold_index), "epoch_best_auc" + str(opt.fold_index) + ".ckpt")
    opt.pretrain_path_allone = pretrain_path_allone  

    opt.model = "resnet"
    resNet = AllInOne_Dist(opt)
    # params = [{'params': parameters}]
    # print (resNet)
    freeze_layers = ["AllInOne"]
    ...
    for epoch in range(opt.epochs): #200
        ...
        for i, (inputs, inputs_heart, inputs_gut, labels, _) in enumerate(train_dataloader):
            ...
            outputs, log_x4, x_hidden_2, out_global = resNet(inputs, inputs_heart, inputs_gut)
            criterion = nn.CrossEntropyLoss()

            # # 1. sample level 
            sample_level = x_hidden_2 @ out_global.T
            loss_sample_level = criterion(sample_level, cos_labels)

            # # 2. reponse level 
            T = 2.0
            outputs_b_log_softmax  = F.log_softmax(outputs, dim=1)
            distillation_loss = F.kl_div(outputs_b_log_softmax, log_x4, reduction='batchmean') * (T * T)

            # # 3. group level
            all_0 = out_global[labels == 0]
            all_1 = out_global[labels == 1]
            all_mean_0 = all_0.mean(dim=0)
            all_mean_1 = all_1.mean(dim=0)
            all_mean_0 = all_mean_0.squeeze()
            all_mean_1 = all_mean_1.squeeze()
            all_mean_0 = all_mean_0 / all_mean_0.norm()
            all_mean_1 = all_mean_1 / all_mean_1.norm()

            brain_0 = x_hidden_2[labels == 0]
            brain_1 = x_hidden_2[labels == 1]
            brain_mean_0 = brain_0.mean(dim=0)
            brain_mean_0 = brain_mean_0.squeeze()
            brain_mean_1 = brain_1.mean(dim=0)
            brain_mean_1 = brain_mean_1.squeeze()
            brain_mean_0 = brain_mean_0 / brain_mean_0.norm()
            brain_mean_1 = brain_mean_1 / brain_mean_1.norm()

            all_norm = out_global / out_global.norm(dim=1, keepdim=True)
            brain_norm = x_hidden_2 / x_hidden_2.norm(dim=1, keepdim=True)

            all_matrix_0 = torch.matmul(all_norm, all_mean_0)
            all_matrix_1 = torch.matmul(all_norm, all_mean_1)

            brain_matrix_0 = torch.matmul(brain_norm, brain_mean_0)
            brain_matrix_1 = torch.matmul(brain_norm, brain_mean_1)
            
            mse_0 = torch.mean((all_matrix_0 - brain_matrix_0) ** 2)
            mse_1 = torch.mean((all_matrix_1 - brain_matrix_1) ** 2)

            loss = criterion(outputs, labels) + 0.3 * loss_sample_level + 0.3 * distillation_loss + 0.3 * (mse_0 + mse_1)
